var Pareidoloop = new function() {
    ...
    var lastImprovedGen;
    var faceA, faceB;
    var canvasA, canvasB, canvasOut, scoreA, scoreB, ctx, ctxA, ctxB, cvs;
    var outputCallback;
    ...
    this.start = function(args) {
  	...
  	outputCallback = args.outputCallback;
        ...
        WebGL2D.enable(canvasA);        
        ctxA = canvasA.getContext("webgl-2d");
        WebGL2D.enable(canvasB);        
        ctxB = canvasB.getContext("webgl-2d");
        WebGL2D.enable(canvasOut);        
        ctx = canvasOut.getContext("webgl-2d");
    ...
    var Face = function(quads) {
               ...
               this.measureFitness = function(canvas) {
                   ...
                   if (comp.length == 1) {
                       comp[0].x -= canvas.width;
                       comp[0].y -= canvas.height;
